...
// ── RECEIPT MODAL (shown in-app after sale) ──
function showReceiptModal(sale, items, paymentInfo, customer) {
  ...
  const receipt58 = generateReceipt58(sale, items, paymentInfo);
  const receipt80 = generateReceipt80(sale, items, paymentInfo, customer);
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:700px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:scaleIn .25s ease;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <h3 style="font-family:'Inter',sans-serif;font-size:18px;font-weight:700;color:#111827;">🧾 Sale Complete!</h3>
      ...
      <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin-bottom:16px;overflow-x:auto;">
        <div style="font-size:12px;color:#6b7280;margin-bottom:8px;">📄 58mm Receipt Preview</div>
        <div style="max-width:320px;margin:0 auto;">${receipt58}</div>
      </div>

      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button onclick="printReceipt(\`${encodeURIComponent(receipt58)}\`,'58mm')" style="flex:1;min-width:120px;padding:12px;background:#0284C7;color:#fff;border:none;border-radius:8px;font-family:'Inter',sans-serif;font-size:13px;font-weight:600;cursor:pointer;">
          <i class="bi bi-printer"></i> Print 58mm
        </button>
        <button onclick="printReceipt(\`${encodeURIComponent(receipt80)}\`,'80mm')" style="flex:1;min-width:120px;padding:12px;background:#059669;color:#fff;border:none;border-radius:8px;font-family:'Inter',sans-serif;font-size:13px;font-weight:600;cursor:pointer;">
          <i class="bi bi-printer"></i> Print 80mm
        </button>
        <button onclick="shareReceiptText(\`${encodeURIComponent(receipt58)}\`)" style="flex:1;min-width:80px;padding:12px;background:#7C3AED;color:#fff;border:none;border-radius:8px;font-family:'Inter',sans-serif;font-size:13px;font-weight:600;cursor:pointer;">
...
function shareReceiptText(receiptContent) {
  const text = decodeURIComponent(receiptContent).replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
  if (navigator.share) {
